...
Hover over the data points to see the fluctuations!

I observed a cylical pattern where the past three day average number of flights was lower on Sunday, Monday, and Tuesday and higher on Wednesday, Thursday, Friday, and Saturday. This means that when weekend flight numbers are rolled into the averages, the average is lower.
...
Secondly, I looked at the overall break down of number of flights per week day. The weekend has a slightly larger number of total flights, however, the weekdays are all about even and not far off from the weekend values. 

This tells me that even though Saturday and Sunday have higher flight numbers, the past three day averages for Sunday, Monday, and Tuesday get lowered either by a lower week day value and/or week day values are more consistent throughout the entire month compared to weekends. This could be possibly due to airlines having consistently planned business travel every week compared to inconsistent summer weekend travel. 
...
In this next section, I wanted to dive deeper into airline trends. I specifically wanted to see if there was a relationship between an airline having the maximum departure delay and the maximum early departure.

I did this by plotting the ranking of the airlines with the largest maximum early departure on the x-axis (ranking of 1 being the largest early departure) and the maximum departure delay value on the y-axis. Each point represents an indiviudal airline that you can see by hoving over the data point.

```{r}
# Combine data sets together
early_rank = 1:17
max_early$early_rank <- early_rank
combined_data <- inner_join(max_delay, max_early, by = "name")


# Create scatter plot
my_scatterplot <- combined_data |> ggplot(aes(x=early_rank, y=Max_Delay, color = name)) +
geom_point(shape = 18) +
theme_minimal()+
labs(x = "Maximum Early Departure Ranking", y = "Maximum Flight Delay (Min)", color = "Airline", title = "Airline Maximum Delay vs Early Departure") + 
...
```{r}
cor(combined_data$early_rank, combined_data$Max_Delay)
```

The pearson correlation coefficient value reveals there is virtually no correlation at all between maximum early departure ranking and maximum departure delay. This could be potentially very important information for travelers when booking since we can't confidently say that an airline with the lowest maximum delay also has the highest maximum early departure (and therefore shouldn't be a deciding factor when choosing an airline).

## Part 4: Airport Deep Dive 

Looking specifically at airports and airlines, I wanted to see which airport had the highest average delay for each airline. It turns out there's a lot of variability in airports as seen below!
...
ExpressJet Airlines and Asheville Regional Airport have the highest delay per airline/airport combination. However, it's also interesting to see that both Raleigh and Indianapolis are the highest delay airport for multiple airlines, indicating these airports might be busy traveler or connecting hubs that frequently experience delays.
